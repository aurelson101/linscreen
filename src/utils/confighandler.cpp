...
bool verifyLaunchFile()
{
    ...
    bool res =
      bootUpSettings.value("LinScreen").toString() ==
...
#else
  : m_settings(qApp->applicationDirPath() + "/linscreen.ini",
               QSettings::IniFormat)
#endif
{
...
void ConfigHandler::setStartupLaunch(const bool start)
{
        ...
        process.start("osascript",
                      QStringList()
                        ...
                        << "tell application \"System Events\" to make login "
                           ...
                           "\"LinScreen\",path:\"/Applications/"
                           "linscreen.app\", hidden:false}");
    } else {
        process.start("osascript",
                      QStringList() << "-e"
                                    << "tell application \"System Events\" to "
                                       "delete login item \"LinScreen\"");
    }
    ...
    QFile file(path + "LinScreen.desktop");
    if (start) {
    ...
    QSettings bootUpSettings(
      "HKEY_CURRENT_USER\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run",
      QSettings::NativeFormat);
    // set workdir for linscreen on startup
    ...
    if (start) {
        QString app_path =
          QDir::toNativeSeparators(QCoreApplication::applicationFilePath());
        bootUpSettings.setValue("LinScreen", app_path);

        // set application workdir
        bootUpPath.beginGroup("linscreen.exe");
        bootUpPath.setValue("Path", QCoreApplication::applicationDirPath());
        bootUpPath.endGroup();

    } else {
        bootUpSettings.remove("LinScreen");
...
QString ConfigHandler::shortcut(const QString& actionName)
{
    ...
    if (!m_settings.contains(setting)) {
        // The action uses a shortcut that is a linscreen default
 ...
 * is the linscreen default (not because the user explicitly configured it), and
...
bool ConfigHandler::checkShortcutConflicts(AbstractLogger* log) const
{
    ...
    for (auto key1 = shortcuts.begin(); key1 != shortcuts.end(); ++key1) {
        for (auto key2 = key1 + 1; key2 != shortcuts.end(); ++key2) {
            ...
            //   user wants to use linscreen's default shortcut for the action
