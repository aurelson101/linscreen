...
LinScreen* LinScreen::instance()
{
    static LinScreen c;
    return &c;
}

CaptureWidget* LinScreen::gui(const CaptureRequest& req)
{
    ...
    // to switch back to the LinScreen Desktop manually. It is not obvious and a
...
void LinScreen::screen(CaptureRequest req, const int screenNumber)
{
    if (!resolveAnyConfigErrors()) {
        return;
    }
...
void LinScreen::full(const CaptureRequest& req)
{
    if (!resolveAnyConfigErrors()) {
        return;
    }
    ...
    if (ok) {
        QRect selection; // `linscreen full` does not support region selection
...
#if defined(Q_OS_MACOS)
void LinScreen::onWindowVisibilityChanged(QWindow::Visibility newVisibility)
{
...
void LinScreen::showDockIcon(QWidget* w)
{
    ...
    connect(qw,
            ...
            this,
            &LinScreen::onWindowVisibilityChanged);
}
#endif

void LinScreen::openSavePath()
{
...
QVersionNumber LinScreen::getVersion()
{
    return QVersionNumber::fromString(
      QStringLiteral(APP_VERSION).replace("v", ""));
}

void LinScreen::setOrigin(Origin origin)
{
    m_origin = origin;
}

LinScreen::Origin LinScreen::origin()
{
    return m_origin;
}
 ...
 */
bool LinScreen::resolveAnyConfigErrors()
{
    bool resolved = true;
...
void LinScreen::requestCapture(const CaptureRequest& request)
{
    if (!resolveAnyConfigErrors()) {
        return;
    }
...
void LinScreen::exportCapture(const QPixmap& capture,
                              QRect& selection,
                              const CaptureRequest& req)
{
    ...
    if (tasks & CR::COPY) {
        LinScreenDaemon::copyToClipboard(capture);
    }

    if (tasks & CR::PIN) {
        LinScreenDaemon::createPin(capture, selection);
        ...
        QObject::connect(
          widget, &ImgUploaderBase::uploadOk, [=, this](const QUrl& url) {
              if (ConfigHandler().copyURLAfterUpload()) {
                  if (!(tasks & CR::COPY)) {
                      LinScreenDaemon::copyToClipboard(
...
void LinScreen::setExternalWidget(bool b)
{
    m_haveExternalWidget = b;
}
bool LinScreen::haveExternalWidget()
{
    return m_haveExternalWidget;
}

// STATIC ATTRIBUTES
LinScreen::Origin LinScreen::m_origin = LinScreen::DAEMON;
