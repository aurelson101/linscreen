...
The developer documentation is served from the official LinScreen website
[linscreen.org][website]. Here's how.
...
GitHub action called [deploy-dev-docs][] in the [linscreen][] repo. This action
will build and deploy this documentation into the `docs/dev` subdirectory of the
[gh-pages][] branch.

### The deploy-dev-docs GitHub workflow

This workflow checks out the linscreen website [repo][website-repo] and does the following:
...
workflow in the [linscreen repo][linscreen], the workflow needs to use an access
token, which it obtains from the `TOKEN_PUSH_TO_WEBSITE_REPO` secret.

The following process was used to set it up:

1. A linscreen organization member with write access must create a personal
...
2. A secret named `TOKEN_PUSH_TO_WEBSITE_REPO` must be added to the
   [linscreen][] repo and its value must be set to the PAT. This can be done
   [here][action-secrets].
